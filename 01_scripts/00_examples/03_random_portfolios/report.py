...
def main():
    ...
    all_runs = qf.Run.load_runs_by_agent(
        ...
        exclude_phases=["EVAL", "TRAIN"],  # Explicitly exclude TRAIN_EVAL
        remove_seed_from_name=False,  # Set to False to keep seed information in names
    )
    ...
    if frames:
        ...
        qf.PlotFrame.plot_confidence_balance(
            frames,
            mean_of_level=["run"],
            plot_config=qf.PlotConfig(matplotlib=qf.MatplotlibConfig(linewidth=1)),
        )
        ...
        combined_frame.plot_cumulative_rewards()
