...
class PPOAgent(SB3Agent):
    def __init__(self, env, config: Optional[PPOConfig] = None):
        ...
        """
        self.config = config or PPOConfig.get_default_config()
    ...
    def get_default_config():
        return PPOConfig.get_default_config()

    @staticmethod
    def get_hyperparameter_space(trial: optuna.Trial):
        return PPOConfig.get_hyperparameter_space(trial)
...
def train_PPO_with_TD_error_logging(self) -> None:
    ...
    self.env.envs[0].env.env.experiment_logger.log_scalar(
        "TRAIN_model_loss/entropy_loss", np.mean(entropy_losses), step=self._n_updates
    )
    self.env.envs[0].env.env.experiment_logger.log_scalar(
        "TRAIN_model_loss/policy_gradient_loss",
        np.mean(pg_losses),
        step=self._n_updates,
    )
    self.env.envs[0].env.env.experiment_logger.log_scalar(
        "TRAIN_model_loss/value_loss", np.mean(value_losses), step=self._n_updates
    )
    self.env.envs[0].env.env.experiment_logger.log_scalar(
        "TRAIN_model_loss/10*log(TD_Error)",
        10 * np.log10(np.mean(td_errors)),
        step=self._n_updates * self.n_epochs,
    )
    self.env.envs[0].env.env.experiment_logger.log_scalar(
        "TRAIN_model_loss/approx_kl", np.mean(approx_kl_divs), step=self._n_updates
    )
    self.env.envs[0].env.env.experiment_logger.log_scalar(
        "TRAIN_model_loss/clip_fraction", np.mean(clip_fractions), step=self._n_updates
    )
    self.env.envs[0].env.env.experiment_logger.log_scalar(
        "TRAIN_model_loss/loss", loss.item(), step=self._n_updates
    )
    self.env.envs[0].env.env.experiment_logger.log_scalar(
        "TRAIN_model_loss/explained_variance", explained_var, step=self._n_updates
    )
    if hasattr(self.policy, "log_std"):
        self.env.envs[0].env.env.experiment_logger.log_scalar(
            "TRAIN_model_loss/std",
            th.exp(self.policy.log_std).mean().item(),
            step=self._n_updates,
        )

    self.env.envs[0].env.env.experiment_logger.log_scalar(
        "TRAIN_model_loss/n_updates", self._n_updates, step=self._n_updates
    )
    self.env.envs[0].env.env.experiment_logger.log_scalar(
        "TRAIN_model_loss/clip_range", clip_range, step=self._n_updates
    )
    if self.clip_range_vf is not None:
        self.env.envs[0].env.env.experiment_logger.log_scalar(
            "TRAIN_model_loss/clip_range_vf", clip_range_vf, step=self._n_updates
        )
