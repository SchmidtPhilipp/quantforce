...
# Metrics class
class Metrics:
    def __init__(self, periods_per_year, risk_free_rate):
    ...
    def append(self, balances):
        ...
        if isinstance(balances, list):
            if isinstance(balances[0], torch.Tensor):
                balances = [balance.cpu().numpy() for balance in balances]
        else:
            balances = balances.cpu().numpy()

        # balances = balances.squeeze()
        returns = Metrics.calculate_returns(balances)
        drawdown = Metrics.max_drawdown(balances)
        annual_ret = Metrics.annualized_return(
            balances, periods_per_year=self.periods_per_year
        )

        self.metrics["sharpe"].append(
        ...
        self.metrics["sortino"].append(
            Metrics.sortino_ratio(returns, risk_free_rate=self.risk_free_rate)
        )
        self.metrics["drawdown"].append(drawdown)
        self.metrics["volatility"].append(
            Metrics.volatility(returns, periods_per_year=self.periods_per_year)
        )
        self.metrics["cumulative"].append(Metrics.cumulative_return(balances))
        self.metrics["annualized"].append(annual_ret)
        self.metrics["calmar"].append(Metrics.calmar_ratio(annual_ret, drawdown))
